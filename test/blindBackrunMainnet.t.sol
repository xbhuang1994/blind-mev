...
contract BlindBackrunTest is Test {
    ...
    function test_arbitrageCalculation() public view {
        ...
        IPairReserves.PairReserves memory secondPairData = getFakePairData(
            false
        );
        uint amountIn2 = blindBackrun.getAmountIn(
            firstPairData,
            secondPairData
        );
    ...
    function test_arbitrageCalculationFlipped() public view {
        IPairReserves.PairReserves
            memory firstPairData = getFlippedFakePairData(true);
        IPairReserves.PairReserves
            memory secondPairData = getFlippedFakePairData(false);
        uint amountIn2 = blindBackrun.getAmountIn(
            firstPairData,
            secondPairData
        );
    ...
    function test_createPairAndArb() public {
        ...
        newToken.approve(address(uniswapv2Router), 1e18);

        newToken.approve(address(sushiswapRouter), 10e18);
        ...
        address firstPair = uniswapFactory.getPair(
            address(newToken),
            wethTokenAddress
        );
        address secondPair = sushiswapFactory.getPair(
            address(newToken),
            wethTokenAddress
        );
    ...
    function test_createPairAndArbSmallDecimals() public {
        ...
        newToken.approve(address(uniswapv2Router), 1e4);

        newToken.approve(address(sushiswapRouter), 10e4);
    ...
    function test_createPairAndArbLargeDecimals() public {
        ...
        newToken.approve(address(uniswapv2Router), 1e20);

        newToken.approve(address(sushiswapRouter), 10e20);
    ...
    function getFlippedFakePairData(
        bool first
    ) internal pure returns (IPairReserves.PairReserves memory) {
        uint256 reserve0;
        uint256 reserve1;
        if (first) {
        ...
        return
            IPairReserves.PairReserves(reserve0, reserve1, price, isWETHZero);
    }

    function getFakePairData(
        bool first
    ) internal pure returns (IPairReserves.PairReserves memory) {
        uint256 reserve0;
        uint256 reserve1;
